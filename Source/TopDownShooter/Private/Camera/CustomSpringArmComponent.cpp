...
void UCustomSpringArmComponent::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime)
{
	...
	if (bDoLocationLag)
	{
		...
		{
			if (bEnableCustomCameraLagSpeedForAxes)
			{
				DesiredLoc.X = FMath::FInterpTo(PreviousDesiredLoc.X, DesiredLoc.X, DeltaTime, CameraAxesLagSpeed.X);
				//DesiredLoc.Y = LerpTarget.Y;
				//DesiredLoc.Z = LerpTarget.Z;
				DesiredLoc.Y = FMath::FInterpTo(PreviousDesiredLoc.Y, DesiredLoc.Y, DeltaTime, CameraAxesLagSpeed.Y);
				DesiredLoc.Z = FMath::FInterpTo(PreviousDesiredLoc.Z, DesiredLoc.Z, DeltaTime, CameraAxesLagSpeed.Z);
			}
			else
			{
